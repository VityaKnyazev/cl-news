...
<p>
После авторизации пользователю возвращается сгенерированный Bearer токен, который необходимо
...
<li>
* Настроен SpringCloudConfig (вынесен в отдельный сервис (config) и настроены разрабатываемые 
сервисы (news, user) на получение config properties в зависимости от профиля).
</li>
...
<ol>
<li>Билдим главное новостное приложение news app: .\gradlew clean build</li>
<li>Запускаем news app в docker: docker-compose up -d</li>
</ol>

<p>P.S. Для корректной работы news app необходимы запущенные spring cloud config server и 
user microservice (см. config, users)!
</p>

<ol>
<li>Примеры GET ендпоинтов:</li>
<p>http://localhost:8080/news</p>
<p>http://localhost:8080/news?page=2&size=1</p>
<p>http://localhost:8080/news?page=2&size=3&sort=title,desc</p>
<p>http://localhost:8080/news?text_part=нас</p>
<p>http://localhost:8080/news?text_part=нас&page=2</p>

<p>http://localhost:8080/comments</p>
<p>http://localhost:8080/comments?page=2&size=1</p>
<p>http://localhost:8080/comments?page=3&size=3&sort=text,desc</p>
<p>http://localhost:8080/comments?text_part=нас</p>
<p>http://localhost:8080/comments?text_part=нас&page=3</p>

<li>POST, PUT, DELETE ендпоинты:</li>
<p>http://localhost:8080/news</p>
<p>http://localhost:8080/comments</p>

<li>POST - authentication, registration ендпоинты:</li>
<p>http://localhost:8080/login<p>
<p>http://localhost:8080/signup<p>
</ol>
